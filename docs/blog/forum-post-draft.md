...
为了能让外部 Agent（Codex、OpenCode 之类）更顺手地操作思源，我写了 [`@frostime/siyuan-cli`](https://github.com/frostime/siyuan-cli)。这个项目发布到了 npm（GPL-3.0），当前 v0.6，还算 beta 级别，拿来尝鲜可以。
...
`effect` 有三种：`deny` 是硬拦截、`allow` 是放行、`confirm` 是暂停等待 `--yes` 参数才放行。`confirm` 适合写操作：Agent 在一般场景下自动跑，但遇到敏感写入时暂停，让人确认一下再继续。
...
```bash
siyuan doc list          # 列出所有文档，附带真实文件路径
