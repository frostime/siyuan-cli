...
export function installSkill(
    name: string,
    opts: { target?: string; dest?: string; force?: boolean; dryRun?: boolean }
) {
    ...
    const operations = [
        { op: 'copy', from: entry.path, to: targetDir },
        { op: 'template', target: targetDir }
    ];
    if (opts.dryRun) {
...
export function uninstallSkill(
    name: string,
    opts: { target?: string; dest?: string }
) {
    const targetDir = skillInstallTarget(name, opts.target, opts.dest);
