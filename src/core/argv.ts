...
function resolveInputSource(
  ...
  // @stdin or "-"
  if ((value === "@stdin" || value === "-") && allowedSources.includes("stdin")) {
    return readStdin();
  }
  ...
  if (value.startsWith("@env:") && allowedSources.includes("env")) {
    ...
    if (envVal === undefined) {
      throw new CliError(ExitCode.GENERAL, "ENV_NOT_SET", `Environment variable ${varName} is not set (for --${field}).`);
    }
    return envVal;
  }
...
export function parsePayload(opts: ParsePayloadOptions): Record<string, unknown> {
  ...
  if (jsonArg) {
    try {
      ...
      throw new CliError(ExitCode.GENERAL, "INVALID_JSON", `Invalid JSON in --json: ${jsonArg}`);
    }
  }
  ...
  if (fileArg) {
    ...
    if (fileArg === "-") {
      ...
      try {
        ...
        throw new CliError(ExitCode.GENERAL, "FILE_READ_ERROR", `Cannot read payload file: ${fileArg}`);
      }
    }
    try {
      base = JSON.parse(content) as Record<string, unknown>;
    } catch {
      throw new CliError(ExitCode.GENERAL, "INVALID_JSON", `Invalid JSON in file ${fileArg}`);
    }
  }
  ...
  if (validate) {
    ...
    if (!valid) {
      const errors = ajv.errors!.map((e) => `${e.instancePath || "payload"} ${e.message}`).join("; ");
      throw new CliError(ExitCode.GENERAL, "PAYLOAD_INVALID", `Payload validation failed: ${errors}`);
    }
  }
...
export function buildEndpointHelp(endpoint: {
  ...
  for (const [field, propSchema] of Object.entries(props)) {
    ...
    const defaultStr = propSchema.default !== undefined ? ` (default: ${JSON.stringify(propSchema.default)})` : "";
    lines.push(`  --${field}  <${propSchema.type ?? "string"}>  ${req}${primary}${defaultStr}`);
