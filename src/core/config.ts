...
function migrateLegacyWindowsConfig(targetPath: string): void {
  ...
  process.stderr.write(JSON.stringify({ notice: "CONFIG_MIGRATED", from: legacyPath, to: targetPath }) + "\n");
  try {
...
export function saveConfig(config: AppConfig, configPath?: string): void {
  ...
  writeFileSync(path, stringify({ ...config, schemaVersion: SCHEMA_VERSION }), "utf-8");
  try {
...
function resolveTokenSource(source?: TokenSource): string | undefined {
  ...
  if (source.type === "file") return readFileSync(source.value, "utf-8").split(/\r?\n/, 1)[0]?.trim();
  if (source.type === "command") return execSync(source.value, { encoding: "utf-8" }).trim();
  return undefined;
}

export function resolveWorkspace(config: AppConfig, overrides: WorkspaceOverrides = {}): ResolvedWorkspace {
  ...
  const name = overrides.workspace ?? process.env["SIYUAN_CLI_WORKSPACE"] ?? config.current;
  if (!name) {
    throw new CliError(
      ExitCode.CONFIG,
      "NO_WORKSPACE",
