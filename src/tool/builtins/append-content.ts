...
export const tool: ToolSchema = {
    ...
    input: {
        ...
        properties: {
            targetId: {
                type: 'string',
                description: 'Notebook ID for dailynote, or document/block ID'
            },
            targetType: {
                type: 'string',
    ...
    async run(ctx, input) {
        const { targetId, targetType, markdown } = input as {
            targetId: string;
            ...
            if (targetType === 'dailynote') {
                return {
                    ...
                    details: {
                        ...
                        operations: [
                            {
                                endpoint: 'filetree.createDailyNote',
                                payload: { notebook: targetId }
                            },
                            {
                                endpoint: 'block.appendBlock',
                                payload: {
        ...
        if (targetType === 'dailynote') {
            ...
            const daily = await ctx.callEndpoint<
                { id?: string } | string | null
            >('filetree.createDailyNote', { notebook: targetId });
            if (
                daily &&
                typeof daily === 'object' &&
                typeof daily.id === 'string'
            ) {
                actualTargetId = daily.id;
            } else if (typeof daily === 'string' && daily.length > 0) {
                const rows = await ctx.callEndpoint<Array<{ id: string }>>(
                    'query.sql',
                    {
                ...
                if (rows.length === 0) {
                    throw new Error(
                        `createDailyNote returned path ${daily} but no matching block was found.`
                    );
                }
                actualTargetId = rows[0]!.id;
            } else {
                throw new Error(
                    `createDailyNote returned an unexpected value for notebook ${targetId}.`
                );
            }
        }
